...
const TaskCalendar = ({ projectId }: TaskCalendarProps) => {
  ...
  const selectedDateTasks = tasks.filter(task => 
    task.task_date === selectedDate.toISOString().split('T')[0]
  );
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2 flex-1">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => handleCompletionToggle(task, checked as boolean)}
                    ...
                    {(task.start_time || task.end_time) && (
                      <div className="flex items-center gap-2 text-gray-400 text-sm pl-6">
                        ...
                        {task.start_time && task.end_time ? (
                          ...
                          <span>Ends at {task.end_time}</span>
                        )}
                      </div>
                    )}
